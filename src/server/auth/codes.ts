...
/** Requests per phone and per address, refilled slowly: brute force is not an option. */
export const REQUEST_CODE_LIMITS = {
  perPhone: createRateLimiter({ capacity: 3, refill: 3, perMs: 10 * 60 * 1000 }),
  perAddress: createRateLimiter({ capacity: 20, refill: 20, perMs: 10 * 60 * 1000 }),
};
export const VERIFY_CODE_LIMITS = {
  perPhone: createRateLimiter({ capacity: 10, refill: 10, perMs: 10 * 60 * 1000 }),
  perAddress: createRateLimiter({ capacity: 60, refill: 60, perMs: 10 * 60 * 1000 }),
};
...
export function requestCode(input: RequestCodeInput): RequestCodeResult {
  ...
  takeOrThrow(REQUEST_CODE_LIMITS.perAddress, input.address);
  ...
  getSmsSender().send(input.phone, `Your Sideout sign-in code is ${code}. It expires in ${CODE_TTL_MS / 60_000} minutes.`);
...
export function verifyCode(input: VerifyCodeInput): VerifyCodeResult {
  const clock = input.clock ?? systemClock;
  takeOrThrow(VERIFY_CODE_LIMITS.perAddress, input.address);
