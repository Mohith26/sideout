...
/** Target for a given set number under best-of rules. */
...
/**
 * A completed set: someone reached the target, won by two, and once past the
 * target the margin is exactly two (deuce play, no cap).
 */
export function judgeSet(teamAPoints: number, teamBPoints: number, target: number): SetVerdict {
  ...
  if (hi > target && hi - lo !== WIN_BY) {
    return { legal: false, reason: `Past ${target} a set ends the moment one side leads by ${WIN_BY}; ${hi}–${lo} cannot happen.` };
  }
...
/**
 * Validate a full scoreline against best-of-1 or best-of-3 rules. Sets must be
 * numbered 1..n contiguously, each set legal for its target, and the match must
 * end exactly when one side reaches the required set count.
 */
export function judgeMatch(sets: readonly SetScore[], bestOf: BestOf): MatchVerdict {
  ...
  if (sets.length > maxSets) return { legal: false, reason: `A best-of-${bestOf} match has at most ${maxSets} set(s).` };

  const ordered = [...sets].sort((x, y) => x.setNumber - y.setNumber);
  let won = { a: 0, b: 0 };
  for (let i = 0; i < ordered.length; i += 1) {
    const set = ordered[i];
    if (!set) return { legal: false, reason: "Missing set." };
...
export function formatSets(sets: readonly Pick<SetScore, "setNumber" | "teamAPoints" | "teamBPoints">[]): string {
  return [...sets]
    .sort((x, y) => x.setNumber - y.setNumber)
