...
/**
 * Two scorelines side by side, the sets on which they differ marked in
 * `fault` with a label — the neutral view a disagreeing submitter, the match
 * page and the organizer's dispute card all share (spec §11.3). Each reading
 * is one column of `A–B` pairs so it fits a 390px screen; no side is called
 * right or wrong, and the copy around it says who settles it.
 */
export interface ScorelineColumn {
...
export interface ScorelineCompareProps {
...
function Pair({ set }: { set: SetScore | undefined }) {
  if (!set) return <span className="text-text-tertiary">—</span>;
  const aWon = set.teamAPoints > set.teamBPoints;
  return (
    <span className="tabular type-mono-stat whitespace-nowrap">
      ...
      <table className="w-full table-fixed border-collapse text-body">
          ...
          <tr className="border-b border-border-subtle">
            ...
            {[left, right].map((col, i) => (
              <th key={i} scope="col" className="px-3 py-2.5 text-start type-label text-text-primary">
                ...
                {col.note ? <span className="mt-0.5 block truncate font-normal text-text-tertiary normal-case tracking-normal">{col.note}</span> : null}
              </th>
            ))}
          </tr>
        </thead>
        <tbody>
          ...
          {setNumbers.map((n) => {
            ...
            return (
              <tr key={n} className={cx("border-b border-border-subtle last:border-b-0", differs && "bg-fault/10")} data-differs={differs || undefined}>
                <th scope="row" className="px-4 py-3 text-start font-medium text-text-primary">
                  <span className="flex flex-col items-start gap-1">
                    <span className="tabular">Set {n}</span>
                    {differs ? (
                      <span className="inline-flex items-center gap-1 rounded-xs border border-fault/40 bg-fault/10 px-1.5 py-0.5 type-label text-fault">
...
export function ScorelineTable({ teamA, teamB, sets, winner, className }: { teamA: string; teamB: string; sets: readonly SetScore[]; winner?: "a" | "b" | null | undefined; className?: string }) {
            ...
            <tr key={side} className="border-b border-border-subtle last:border-b-0">
              <th scope="row" className={cx("px-4 py-3 text-start font-medium", winner === side ? "text-text-primary" : "text-text-secondary")}>
                <span className="inline-flex max-w-full items-center gap-2">
                  ...
                  {winner === side ? <Icons.check size={14} className="shrink-0 text-surf" aria-label="Winner" /> : null}
                </span>
              </th>
              {ordered.map((s) => (
