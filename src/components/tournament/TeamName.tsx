import type { TeamWithMembers } from "@/db/queries/tournaments";
import { cx } from "@/lib/cx";

/** A team as it is announced on the sand, with an optional seed number. */
export function TeamName({ team, seed, className }: { team: TeamWithMembers | null; seed?: boolean; className?: string }) {
  if (!team) return <span className={cx("text-text-tertiary", className)}>TBD</span>;
  return (
