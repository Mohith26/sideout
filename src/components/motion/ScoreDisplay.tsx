...
import { motionDurationMs, prefersReducedMotion } from "@/components/motion/reduced-motion";
import { cx } from "@/lib/cx";

/**
 * A score that rolls to its new value (spec §12.4, transition 1): tabular
 * digits so the width never jitters, the number counted from the previous
 * value to the new one over --d-base with an ease-out, and the settled glyphs
 * fading in over the last beat. The first render shows the value as is; only
 * a change rolls. Under reduced motion the number changes at once and only
 * the fade remains.
 */
