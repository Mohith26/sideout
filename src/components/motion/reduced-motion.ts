/**
 * The one place the motion preference and the motion tokens are read from
 * the browser, so every JavaScript-driven transition (the count-up, the FLIP)
 * takes the same branch the stylesheet takes. Safe on the server: no window
 * means no preference and the token defaults.
 */
...
export function motionToken(name: keyof typeof TOKEN_DEFAULTS | string): string {
...
export function motionDurationMs(name: "--d-micro" | "--d-base" | "--d-enter" | "--d-draw"): number {
  ...
  if (!match) return parseFloat(TOKEN_DEFAULTS[name] ?? "0") || 0;
