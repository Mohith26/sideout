...
import { flipMoves, playFlip, snapshotRows, type FlipSnapshot } from "@/components/motion/flip";
import { motionDurationMs, motionToken, prefersReducedMotion } from "@/components/motion/reduced-motion";

/**
 * Wraps server-rendered rows that carry `data-team-id` (and `data-rank`) and
 * plays the leaderboard reorder (spec §12.4, transition 2) whenever they
 * re-render in a different order: the positions from the previous commit are
 * the "first", the positions after this commit are the "last", and each moved
 * row slides from one to the other. The rows themselves stay server-rendered;
 * nothing here holds a second copy of the standings.
 */
export interface FlipRowsProps extends HTMLAttributes<HTMLDivElement> {
  children: ReactNode;
