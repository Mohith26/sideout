...
/**
 * A pool sheet, the way it is pinned to the tent pole on the sand: teams down
 * the side, opponents across the top, each cell the result of that meeting
 * (or when it happens), and the record at the end. Every cell is derived from
 * match and set rows; a result cell links to the match.
 */
...
function cellText(cell: Cell, timeZone: string): { text: string; title: string; tone: string } {
  ...
  switch (match.status) {
    ...
    case "in_progress":
      return { text: mine.length ? `${mine[mine.length - 1]}–${theirs[theirs.length - 1]}` : "Live", title: `In progress${full ? `, ${full}` : ""}`, tone: "text-surf" };
            ...
            <tr className="border-y border-border-subtle">
              ...
              {order.map((t) => (
                <th key={t.id} scope="col" className="px-1.5 py-2 text-center type-label whitespace-nowrap text-text-tertiary" abbr={t.name}>
                  ...
                  {t.seed !== null ? <span className="tabular">{t.seed} </span> : null}
                  {shortName(t.name)}
                </th>
              ))}
          ...
          <tbody>
            {order.map((row) => (
              <tr key={row.id} data-team-id={row.id} className={cx("border-b border-border-subtle last:border-b-0", row.id === highlightTeamId && "bg-bg-overlay")}>
                <th scope="row" className="max-w-[7.5rem] px-3 py-2 text-start font-medium text-text-primary sm:max-w-none sm:px-4">
                  <span className="flex min-w-0 items-baseline gap-2">
                    {row.seed !== null ? <span className="tabular type-label text-text-tertiary">{row.seed}</span> : null}
                    <span className="truncate">{row.name}</span>
                  </span>
                  {row.members.length ? <span className="hidden truncate type-label text-text-tertiary sm:block">{row.members.map((m) => m.displayName.split(" ")[0]).join(" & ")}</span> : null}
                </th>
                {order.map((col) => {
                  if (col.id === row.id) {
                    return (
                  ...
                  const content = (
                    <span title={title} className={cx("tabular inline-flex min-h-11 min-w-10 items-center justify-center rounded-xs px-1 type-mono-stat whitespace-nowrap", tone)}>
                      {text}
                    </span>
                  );
                  return (
