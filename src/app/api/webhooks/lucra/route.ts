...
import { handle, NO_STORE, readRawBody } from "@/server/http";
import { receiveLucraWebhook, WEBHOOK_BODY_LIMIT_BYTES } from "@/server/lucra-webhooks";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/**
 * POST /api/webhooks/lucra (spec §7.6). The raw body is read — capped, since
 * the caller is anonymous — before anything parses it; verification,
 * deduplication, persistence and processing live in `@/server/lucra-webhooks`.
 * The response never carries a secret or the expected digest — only what
 * happened to the delivery.
 */
export async function POST(request: NextRequest) {
  return handle(async () => {
    const rawBody = await readRawBody(request, WEBHOOK_BODY_LIMIT_BYTES);
