...
/**
 * The score outbox (spec §11: a queued score submission that survives reload
 * and syncs on reconnect). A submission the phone cannot send is stored in
 * IndexedDB exactly as it would have gone over the wire — the same
 * `POST /api/matches/:id/scores` body — and replayed later through the same
 * route, so the server treats it exactly like a live one: same validation,
 * same consensus path. Nothing is judged on the phone.
 *
 * Pure queue logic over a small store interface; `IndexedDbOutbox` is the
 * browser store and `MemoryOutbox` the one tests and non-IDB environments
 * use. One queued item per match: a newer submission from this phone
 * replaces the older one, the same way the server supersedes a team's earlier
 * row.
 */
...
export interface ReplayOutcome {
  item: OutboxItem;
  result: "sent" | "failed" | "settled_elsewhere" | "deferred";
  message: string | null;
}

export interface ReplayReport {
  outcomes: ReplayOutcome[];
  /** Whether a queued item is still waiting (the network did not answer). */
  pending: boolean;
}

/** Refusals that mean the match no longer takes this submission: the item is dropped, not shown as a failure. */
const SETTLED_ELSEWHERE = new Set(["match_not_open"]);
...
export function detailCodeOf(result: ApiResult<unknown>): string | null {
...
/**
 * Send every queued item in order through `send`. A transient failure stops
 * the run and leaves the rest queued for the next reconnect; a definitive
 * refusal marks that item `failed` (or drops it when the match has since
 * been settled) and moves on.
 */
export async function replayOutbox(store: OutboxStore, send: OutboxSend, now: () => number = Date.now): Promise<ReplayReport> {
  ...
  for (const item of queued) {
    ...
    try {
      ...
      result = { ok: false, error: { code: "unavailable", message: err instanceof Error ? err.message : "Could not send." }, status: 0, retryAfterMs: null };
    }
    if (result.ok) {
    ...
    const code = detailCodeOf(result) ?? result.error.code;
    if (SETTLED_ELSEWHERE.has(code)) {
      await store.remove(item.id);
      outcomes.push({ item, result: "settled_elsewhere", message: result.error.message });
      continue;
    }
    await store.put({ ...item, status: "failed", attempts: item.attempts + 1, lastError: { code, message: result.error.message, at } });
