...
`lucraSettlementHook` runs `settleTournament`: any consensus still `agreed` is written,
...
| Before the first write to a tournament, `/pool-tournament/query` must return exactly one matchup (7.3.4) | `ensureMatchupTarget` in `src/server/lucra.ts`: caches `lucra_matchup_id` + `lucra_matchup_verified_at`; otherwise raises a blocking `LucraAlert`, moves a live tournament to `awaiting_settlement`, refuses the write | `lucra.test.ts` (duplicate matchup in the mock) |
...
`POST /api/webhooks/lucra` reads `request.text()` first, verifies
...
`TournamentCompleted` (settles an `awaiting_settlement` tournament, confirms a settled
one, alerts on a live one), `TournamentCanceled` (alert), `TournamentUserJoined` and
...
`ignored` with a 2xx. An invalid signature is refused with 401 and kept as evidence
under a key that can never shadow the genuine delivery.
