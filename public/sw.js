/*
 * Sideout service worker (spec §11: installable, offline read of the pages a
 * player has opened). Plain JavaScript on purpose: it is served as-is from
 * public/ and registered by src/components/offline/ServiceWorkerRegistration.tsx
 * with the build sha as `?v=`, which this file reads as its cache version so a
 * new build starts from empty caches and drops the old ones on activation.
 *
 * What it does, and what it deliberately does not:
 *
 * - Precaches the offline page and the manifest at install.
 * - `/_next/static/*` (content-hashed, immutable): cache first.
 * - Page navigations: network first with a short timeout, the copy kept in the
 *   pages cache so the signed-in player's own event, pool and match pages open
 *   without signal; the offline page when nothing is cached.
 * - `GET /api/tournaments/*`, `/api/matches/*`, `/api/me`: network first, cached
 *   for the same offline read.
 * - Nothing else is touched: POSTs (the score outbox in src/lib/offline/ replays
 *   those), Next's RSC navigations (the framework retries them itself once the
 *   connection returns), HEAD connectivity checks, the dev and mock routes, and
 *   every cross-origin request go straight to the network.
 *
 * A sign-out posts `{ type: "clear-pages" }`; the pages and API caches are
 * dropped so a shared phone does not keep the previous player's pages.
 */
...
const PAGE_DENYLIST = [/^\/sign-in/, /^\/api\//, /^\/health$/, /^\/offline$/];
 ...
 * NAVIGATION_TIMEOUT_MS (the fetch keeps going and refreshes the cache when it
 * lands). With no network and no copy, the offline page.
 */
